...
public interface BoardRepository extends JpaRepository<Board, Long>{
    ...
    @Query(value = "select p from Board p where p.ingredients in :include and p.ingredients not in :exclude and p.nation = :nation and p.title like :title order by p.viewCount desc")
    ...
    @Query(value = "select p from Board p where p.ingredients in :include and p.ingredients not in :exclude and p.nation = :nation and p.title like :title order by (size(:include)/size(p.ingredients)) desc")
